...
class MultilingualSentimentAnalyzer:
    ...
    def __init__(self, config_path: str = None, device: str = "auto", use_shared_models: bool = True):
        """
        ...
        Args:
            ...
            use_shared_models: Whether to use shared models for memory optimization
        """
    ...
    def _get_sentiment_model(self):
        """Get sentiment model (shared or individual)"""
        ...
        if self.use_shared_models:
            try:
                ...
                model_name = self.config["models"]["sentiment"]["primary"]
                
                self.sentiment_model = shared_manager.get_shared_model(
                    "multilingual_sentiment", 
                    model_name
                )
                
                if self.sentiment_model is not None:
                    logger.info("Using shared multilingual sentiment model")
                    return self.sentiment_model
                else:
                    logger.warning("Failed to get shared model, falling back to individual loading")
                    
            except Exception as e:
                logger.warning(f"Failed to use shared model: {e}, falling back to individual loading")
    ...
    def _setup_device(self, device: str) -> torch.device:
        """Setup device for inference with optimal allocation."""
        if not HAS_TRANSFORMERS:
            return torch.device("cpu")
            
        if device == "auto":
            if torch.cuda.is_available():
                # Windows/Linux with CUDA - Use GPU for large transformer models (optimal)
                logger.info("CUDA available, using GPU for multilingual transformer (optimal)")
                return torch.device("cuda")
            elif platform.machine() in ['arm64', 'aarch64'] and platform.system() == 'Darwin':
                # Apple Silicon - Force CPU to prevent segmentation faults
                logger.info("Apple Silicon detected - using CPU for stability (multilingual transformer)")
                return torch.device("cpu")
            else:
                logger.info("Using CPU for multilingual transformer")
                return torch.device("cpu")
    ...
    def _create_dummy_model(self):
        ...
        logger.warning("Creating dummy sentiment model as last resort")
        
        class DummySentimentModel:
            def __call__(self, text):
                # Always return neutral sentiment
                return [[
                    {'label': 'LABEL_1', 'score': 1.0}  # Neutral
                ]]
        
        self.sentiment_model = DummySentimentModel()
        logger.info("Dummy sentiment model created")
    ...
    def analyze_sentiment(self, text: str, language: str = None) -> Dict[str, Any]:
        ...
        # Analyze sentiment using shared or individual model
        try:
            ...
            results = sentiment_model(processed_text)
                ...
                for item in results[0]:
                    try:
                        ...
                        if hasattr(score_value, 'item'):
                            scores[item['label'].lower()] = float(score_value.item())
                        else:
                            scores[item['label'].lower()] = float(score_value)
                    except Exception as e:
                        logger.warning(f"Failed to extract score: {e}")
                        scores[item.get('label', 'unknown').lower()] = 0.5
            else:
                # Model returns single prediction
                try:
                    score_value = results[0]['score']
                    if hasattr(score_value, 'item'):
                        scores = {results[0]['label'].lower(): float(score_value.item())}
                    else:
                        scores = {results[0]['label'].lower(): float(score_value)}
                except Exception as e:
                    logger.warning(f"Failed to extract score: {e}")
                    scores = {'neutral': 0.5}
            
            # Ensure we have some scores to work with
            if not scores:
                scores = {'neutral': 1.0}
