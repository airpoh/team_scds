...
class EmotionSarcasmDetector:
    ...
    def _load_models(self):
        ...
        try:
            logger.info(f"Loading emotion detection model on {self.device}...")
            ...
            try:
                ...
                if "meta tensor" in str(e).lower():
                    ...
                    if self.device != "cpu":
                        try:
                            self.emotion_model.model = self.emotion_model.model.to(self.device)
                            logger.info(f"Emotion model moved to {self.device} successfully")
                        except Exception as device_error:
                            logger.warning(f"Could not move emotion model to {self.device}: {device_error}")
                            logger.info("Using emotion model on CPU")
                else:
                    raise e
            
            logger.info(f"Loading sarcasm detection model on {self.device}...")
            # Try loading directly on target device first
            try:
                ...
                logger.info(f"Sarcasm model loaded successfully on {self.device}")
            except Exception as e:
                if "meta tensor" in str(e).lower():
                    logger.warning(f"Meta tensor error on {self.device}, falling back to CPU loading: {e}")
                    # Fallback: load on CPU then move to target device
                    ...
                    if self.device != "cpu":
                        try:
                            self.sarcasm_model.model = self.sarcasm_model.model.to(self.device)
                            logger.info(f"Sarcasm model moved to {self.device} successfully")
                        except Exception as device_error:
                            logger.warning(f"Could not move sarcasm model to {self.device}: {device_error}")
                            logger.info("Using sarcasm model on CPU")
                else:
                    raise e
    ...
    def _load_fallback_models(self):
        ...
        try:
            logger.info(f"Loading fallback models on {self.device}...")
            
            # Try loading emotion fallback model directly on target device
            try:
                self.emotion_model = pipeline(
                    "text-classification",
                ...
                logger.info(f"Fallback emotion model loaded successfully on {self.device}")
            except Exception as e:
                if "meta tensor" in str(e).lower():
                    logger.warning(f"Meta tensor error on {self.device}, falling back to CPU loading: {e}")
                    # Fallback: load on CPU then move to target device
                    ...
                    if self.device != "cpu":
                        try:
                            self.emotion_model.model = self.emotion_model.model.to(self.device)
                            logger.info(f"Fallback emotion model moved to {self.device} successfully")
                        except Exception as device_error:
                            logger.warning(f"Could not move fallback emotion model to {self.device}: {device_error}")
                            logger.info("Using fallback emotion model on CPU")
                else:
                    raise e
            
            # Try loading sarcasm fallback model directly on target device
            try:
                self.sarcasm_model = pipeline(
                    "text-classification",
                ...
                logger.info(f"Fallback sarcasm model loaded successfully on {self.device}")
            except Exception as e:
                if "meta tensor" in str(e).lower():
                    logger.warning(f"Meta tensor error on {self.device}, falling back to CPU loading: {e}")
                    # Fallback: load on CPU then move to target device
                    ...
                    if self.device != "cpu":
                        try:
                            self.sarcasm_model.model = self.sarcasm_model.model.to(self.device)
                            logger.info(f"Fallback sarcasm model moved to {self.device} successfully")
                        except Exception as device_error:
                            logger.warning(f"Could not move fallback sarcasm model to {self.device}: {device_error}")
                            logger.info("Using fallback sarcasm model on CPU")
                else:
                    raise e
