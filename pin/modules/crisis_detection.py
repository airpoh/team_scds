...
class ConsoleAlertManager(AlertManager):
    """Console-based alert manager"""
    
    def send_alert(self, alert: CrisisAlert) -> bool:
        try:
            print(f"CRISIS ALERT [{alert.alert_type}]: {alert.description}")
            print(f"Severity: {alert.severity:.3f} | Confidence: {alert.confidence:.3f}")
            print(f"Affected comments: {alert.get_affected_comments_count()}")
            return True
        except Exception as e:
...
class CrisisDetectionSystem:
    ...
    def _filter_alerts(self, alerts: List[CrisisAlert]) -> List[CrisisAlert]:
        ...
        for alert in alerts:
            ...
            # Check cooldown
            cooldown_key = f"{alert.alert_type}_{alert.timestamp.strftime('%Y-%m-%d_%H')}"
